...
<%@include file="/ndims/ui_if/workflow/proc_templete/actv_proc_list.html" %>
...
<script type="text/javascript">
	...
	// 移除标题行以外的内容
		...
		for (var i = 0; i < list.length; i++) {
			...
			// 各列内容设定
			// 列：撤回
	    html = "<a href=\"javascript:void(0);onClickPullback('"
					+ listRow.systemMatterId + "', '"
					+ listRow.userDataId + "', '" + listRow.flowId
      ...
      tr.append(td);
			...
			// 列：最终处理日
			td = $(document.createElement("td")).html(
					escapeHTML(listRow.finalProcDate), true);
			tr.append(td);
