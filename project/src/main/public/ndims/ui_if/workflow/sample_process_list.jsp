...
<%@include file="/ndims/ui_if/workflow/proc_templete/process_list.html" %>
...
<script type="text/javascript">
	...
	// 移除标题行以外的内容
		...
		for (var i = 0; i < list.length; i++) {
			...
			// 各列内容设定
			// 列：处理
	    html = "<a href=\"javascript:void(0);onClickProcess('"
					+ listRow.systemMatterId + "', '"
					+ listRow.userDataId + "', '" + listRow.nodeId
					+ "', '" + listRow.pageType
					+ "', workflowParams)\">处理</a>";
			...
			// 列：移交
      html = "<a href=\"javascript:void(0);onClickTransfer('"
          + listRow.systemMatterId + "', '"
          + listRow.nodeId
          + "', workflowParams)\">移交</a>";
      var td = $(document.createElement("td")).html(html, true);
      ...
      tr.append(td);
			...
			// 列：节点名
			td = $(document.createElement("td")).html(
					escapeHTML(listRow.nodeName), true);
			tr.append(td);

			// 列：状态
			td = $(document.createElement("td")).html(
					escapeHTML(listRow.status), true);
			tr.append(td);

			// 列：到达日
			td = $(document.createElement("td")).html(
					escapeHTML(listRow.arriveDate), true);
			tr.append(td);

			// 列：处理权限
			td = $(document.createElement("td")).html(
					escapeHTML(listRow.procAuth), true);
			tr.append(td);
  ...
  <div>
    <button type="button" id="process_list">检索未处理（待办）一览</button>
  </div>
